...
function buscarPorLegajo(legajoBuscado, estudiantes) {
   let alumnoEncontrado = estudiantes.find(estudiante => estudiante.legajo === legajoBuscado)
...
function generarMails2(estudiantes) {
  return estudiantes.map(alumno => `${alumno.nombre.toLowerCase()}@universidad.edu.ar`);
}
